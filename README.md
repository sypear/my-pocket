...
#### - 오류 이유 찾기
<a href="https://stackoverflow.com/questions/65941392/why-does-setstate-cause-my-react-app-go-into-infinite-loop">Why does setState cause my React app go into infinite loop?</a> 글을 본 후 추측한 오류 발생 이유는 아래와 같습니다.<br/>
...
<b>props에 변경 사항이 있는 경우에만 호출하고 상태를 업데이트 하는</b> 방식으로 코드를 수정했습니다.<br/><br/>
...
`useEffect`를 이용해서 `props.filteredItems`이 변할 때만 상태를 업데이트 하도록 하니 쓸데없는 재렌더링이 일어나지 않아 무한 루프가 일어나지 않았습니다.

<br/>

## 🌈 느낀 점
### 미리 구조를 생각한 후 기능 구현을 시작하자
...
대략적으로 큰 그림을 그리고 시작하지 않고 바로 구현에 들어가서 컴포넌트를 분리했다가 다시 합쳤다가 또 분리했다가 하는..<br/>
굳이 하지 않아도 될 시간 낭비(~~삽질~~)를 많이 했습니다😂<br/><br/>

#### 두 번째 삽질
8월 4일 기능 구현을 마무리하고 친구에게 Repository를 공유했는데,
<br/>

<img src="https://user-images.githubusercontent.com/105365737/183237615-15002bc0-f773-46a2-9b9f-4b5c086bceb3.jpg" width="400">
이런 피드백을 받았습니다.<br/><br/>

처음에 컴포넌트 명을 Asset과 NewAsset이라고 네이밍을 했었는데요. (아래 이미지 참조)<br/>
![원래2](https://user-images.githubusercontent.com/105365737/183238143-a9c339dc-7236-42f2-8cb7-e4e1c3d8a6f8.png)

React에서 이미지, font등을 저장하는 폴더 명을 주로 assets으로 사용한다는 걸 모른채로 네이밍을 했기 때문에 발생한 문제였습니다.<br/>

그래서 asset이 들어가는 컴포넌트명, 함수명, 변수명을 전부 변경했고..<br/>
그 과정이 정말 힘들었습니다. (discard를 엄청 했습니다😂)<br/>
조금이라도 더 찾아보고 더 생각해봤다면 이런 일은 없었을텐데.. 오늘의 삽질을 바탕으로 앞으론 이러지 말자고 다짐했습니다.
